...
2026-09-11 用户手动加载正式新 worker 后，外部 probe、三证券×三周期读取，以及三组 5 分钟实际下载读回均通过；日线和 1 分钟本轮命中缓存，没有触发下载。详见[正式入口首次实测](research/2026-09-11-auto-worker-live.md)。这更新了此前 GUI 受阻、入口尚未加载的状态，但只属于单日小样本验证；容量、长期稳定性、异常恢复及生产验收仍未完成。本版本仍为 Alpha，不能直接接管生产，也没有实时行情内存通道或交易功能。
