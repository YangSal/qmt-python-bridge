...
## 2026-09-11 正式入口真实终端小样本验证

用户手动运行 `strategy_auto.py` 后，外部 probe 确认自动协议、worker version 3 和下载开关。股票、ETF、指数共九个证券×周期单元读回校验通过；其中三组 5 分钟触发真实下载，各返回 48 条，另外六组日线/分钟线命中缓存。正式外部 facade 读取、正常完成任务的同 ID 复核、本地状态查询均通过。详见[本次实测及边界](research/2026-09-11-auto-worker-live.md)。下方“未加载 / 待实测”描述保留为各历史阶段的状态，不代表当前入口仍未加载。全面终端验收、生产迁移、交易和实时内存通道仍未完成。

## 0.2.0a1 最终源码修订验证

整分支审查提出的 1 项数据校验问题及 2 项说明问题已在 `f19322e` 修复，限定范围复核全部通过。主任务独立复跑完整回归为 **254 passed in 52.23s**，另验证源码导入、8 表字段资源、7 模块 Python 3.6 语法和三种 CLI help 均通过。本轮仅交付源码；维护者已后续明确授权合并并推送 `main`，但这不代表正式新 worker 的真实终端验收或生产切换。
