...
- [ ] CLI `download` 仅在聚合状态 `verified` 时返回 0，其他状态保留报告和 ID；`download-status` 仅读 `client_jobs/`，不发 QMT 请求。
...
- [ ] 确认 `records/`、`states/`、`client_jobs/`、`response_repairs/`、请求/响应、日志、锁和证据没有进入 Git 或 wheel；`response_repairs/` 只被描述为有界修复标记，不是第二请求队列。
