...
实时短采样后的一次状态读取记录最长回调约 155ms；继续执行历史任务后，累计最长回调
达到约 2.18 秒。20ms 只是两次历史调用之间检查的软预算，不能中断已进入 QMT 的
同步调用，不能据此承诺生产实时延迟。
...
恢复后的首个 300 秒批次尝试 349 单元，总体从此前 97 项成功更新至 436 项成功、
10 项失败；再次对账一项过期的只读复权请求后为 437 项成功、9 项失败。
北京时间 10:43 启动其余全市场续采，单次进程上限 23 小时，保留失败项并检查桥健康。
此为阶段快照，不是最终全市场完成数；实际状态以本机 `summary.json` 和分片报告为准。
