...
10 是本项目首轮功能验证设置的保守实现边界，不是从 QMT 实测得出的容量上限，
也不是已完成的全市场实时采集能力。它同时限制单次请求、累计活动/未知订阅以及本地
观察缓存；外部测试消费者也做同样校验，分多次请求不能绕过订阅总量限制。
目前真实终端只完成了 2 只证券的短采样，尚未完成 10 只或更大容量的压力验收。

这项选择与当前有界实现配套：单帧 256KiB、每只原始报价最多 16,000 字节、发送队列
最多 8 帧，并且原生 QMT 调用由单一定时回调串行执行。10 不是这些数字推导出的
QMT 硬上限。扩大范围需要分别设计订阅总量和快照分批量、校验分帧及背压，并实测延迟
和持续运行情况；仅改大常量不能证明全市场可用。当前轮询快照仍不保证捕获每次行情更新。
